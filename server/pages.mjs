...
function detailHTML(data, user) {
  ...
  const feedbackSummary = `<div class="feedback-stats"><span class="feedback-version">v${i.revision} 复现</span>${[["success", "成功"], ["partial", "部分完成"], ["failed", "无法完成"]].map(([key, label]) => `<span><strong>${feedback.find((f) => f.outcome === key)?.count || 0}</strong> ${label}</span>`).join("")}</div>`;
  const discussion = `<div class="discussion">${feedbackSummary}<form class="discussion-form" data-action="comment" ${ref}><label>内容<textarea name="content" required minlength="2" maxlength="2000" rows="3" placeholder="分享使用体验、提出问题，或说说复现结果…"></textarea></label><details class="discussion-extras"><summary>添加复现结果或评分（可选）</summary><div class="discussion-options"><label>复现结果（可选）<select name="outcome"><option value="">不填写</option><option value="success">成功</option><option value="partial">部分完成</option><option value="failed">无法完成</option></select></label><fieldset class="star-rating" data-star-rating><legend>评分（可选）</legend><div class="rating-controls"><div class="rating-options">${[1, 2, 3, 4, 5].map((rating) => `<label class="rating-choice"><input type="radio" name="rating" value="${rating}" aria-label="${rating} 星">${starIcon}</label>`).join("")}</div><span class="rating-label" data-rating-label aria-live="polite">点击星星评分</span><button class="text-button" type="button" data-clear-rating hidden>清除评分</button></div></fieldset></div></details><button class="button" type="submit">发布</button><p class="form-error" role="alert"></p></form><div class="discussion-list">${comments.length ? comments.map((c) => commentCard(c, user?.id)).join("") : '<p class="discussion-empty muted">还没有讨论与反馈，欢迎留下你的使用经验。</p>'}<button class="text-button" data-more-comments ${ref} data-page="2" ${comments.length < 20 ? "hidden" : ""}>加载更多讨论</button></div></div>`;
